...
- *variabili*: $x in ZZ^n$;
- *vincoli*:
  - $x_i + x_j gt.eq 1 quad forall{i,j} in E$, ovvero almeno uno dei vertici di ogni lato deve essere nella soluzione;
  - $x_i gt.eq 0 quad forall i in V$, ovvero impongo la non negatività;
  - $x_i lt.eq 1 quad forall i in V$, ovvero impongo valori booleani alle variabili;
...
Sia $pi : (G = (V,E), w_i)$, allora $ pi arrow.squiggly "ILP"(pi) arrow.squiggly "LP"(pi) = x^* arrow.squiggly rounding(x^*) = overline(x) bar.v overline(x)_i = cases(0 & "se" x_i^* < 0.5, 1 quad & "se" x_i^* gt.eq 0.5) quad forall i in V . $
