...
#proof()[
  Noi paghiamo $ w = sum_(i in "out"_V) w_i $ ma in output ho tutti i vertici non stretti sul pricing, quindi $ w = sum_(i in V bar.v p_e "non" \ "è stretto su" i) w_i $ ma il costo $w_i$ è la somma di tutte le offerte dei lati incidenti, quindi $ w = sum_(i in "out"_V) space sum_(e in E and i in e) p_e . $
...
== Problema dei cammini disgiunti (disjoint paths)
...
I cammini si possono solo allungare, quindi il passaggio di "stato" è da _corto_ a _lungo_.
...
Osserviamo che $ sum_(i in I^* slash I_o) overline(l)(pi_i^*) gt.eq beta^c abs(I^* slash I_o) $ per il primo lemma di questa lezione.

Osserviamo anche che nella soluzione ottima nessun arco è usato più di $c$ volte. Quindi possiamo maggiorare il costo con il costo di tutti gli archi moltiplicati per $c$, ovvero $ sum_(i in I^* slash I_o) overline(l)(pi_i^*) lt.eq c sum_(a in A) overline(l)(a) lt.eq c (beta^(c+1) abs(overline(I)) + m) . $ L'ultima minorazione è per il lemma 2 di questa lezione.
...
#proof()[
  Osserviamo anche che $ beta^c abs(I^*) &lt.eq beta^c abs(I^* slash I_o) + beta^c abs(I^* sect I_o) \ &lt.eq_(o 1) sum_(i in I^* slash I_o) overline(l)(pi_i^*) + beta^c abs(I_o) \ &lt.eq_(o 2) c (beta^(c+1) abs(overline(I)) + m) + beta^c abs(I_o) \ &lt.eq c (beta^(c+1) abs(I_o) + m) + beta^c abs(I_o) . $
